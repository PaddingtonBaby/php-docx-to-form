...
2. **Проверка шаблона** - API принимает только `.docx`, проверяет MIME, размер файла и ZIP-структуру перед распаковкой.
3. **Скрытие переменных** - (скорее всего костыль): установка цвета шрифта текста в белый (#FFF). Именно так текст сохраняет исходную ширину, оставляя разметку для PDF идентичной исходной. Если удалять - начинаются смещения позиций будущих флажков в документе.
4. **Конвертация в PDF** - через LibreOffice headless (`soffice --convert-to pdf`)
5. **Извлечение позиций** - `pdftotext -bbox` возвращает XML с координатами слов. Каждая переменная сопоставляется с полями через разрешение имён.
6. **Генерация JPEG** - `pdftoppm -jpeg` из того же PDF.
7. **Наложение маркеров** - на фронте позиционируются нумерованные кнопки по процентным координатам поверх изображения.
...
├── src/
│   ├── ApiException.php        # API-ошибки с понятными HTTP-кодами
│   ├── DocxConverter.php       # Ядро: процесс конвертации, заполнение
│   ├── FieldExtractor.php      # Извлечение координат полей из PDF
│   └── Security.php            # Лимиты, rate limit, проверка DOCX и пути хранения
└── ../php-docx-to-form-storage/ # Создаётся автоматически вне web-root
    ├── templates/
    ├── output/
    └── rate/
```
...
- Максимальный размер DOCX - 10 МБ.
- Максимальный JSON для заполнения - 512 КБ.
- Загруженные шаблоны и временные результаты хранятся вне web-root в `../php-docx-to-form-storage`.
- Старые файлы автоматически очищаются после 24 часов.
- Лимиты по запросам от IP: конвертация файла шаблона - 20 запросов за 10 минут, заполнение - 60 запросов за 10 минут.
